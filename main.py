...
# Improved UI with a Summary Header
HTML_TEMPLATE = """
<!DOCTYPE html>
<html>
...
<style>
    ...
    .stat-val { font-size: 20px; font-weight: bold; color: #FF3B30; }
...
def process_data(image_bytes):
    ...
    payload = {
        ...
        "messages": [
            {"role": "system", "content": "Return ONLY a JSON list. Each object must have keys 's' (Service), 'a' (Amount - as a number), 'c' (Cancellation Strategy)."},
            {"role": "user", "content": [{"type": "text", "text": "Extract all subscriptions as JSON."}, {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{base64_image}"}}]}
        ]
    }
    
    response = requests.post(URL, headers=headers, json=payload)
    if response.status_code == 200:
        json_text = response.json()['choices'][0]['message']['content'].replace("```json", "").replace("```", "").strip()
